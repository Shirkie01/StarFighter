...
public class NetworkSpaceship : NetworkBehaviour
{
    ...
    private void OnLifeChanged(int newValue)
    {
        lifeCount = newValue;
        UpdateScoreLifeText();
    }
    ...
    public void CreateBullets()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            GameObject bullet = Instantiate(bulletPrefab, _rigidbody.position + offsets[i] + transform.forward * 4, Quaternion.identity) as GameObject;
